...
> Status: **preview (`v0.9.0`)**. The contract kernel, policy gateway, workflow runtime, project-local templates, reconciliation, local Git/build/test adapters, and the host-mediated Unreal MCP adapter are implemented and tested. Full Bootstrap, Feature, and Release execution is not yet claimed for every provider integration.
