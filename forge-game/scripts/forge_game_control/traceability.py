...
TRACEABILITY_GRAPH_SCHEMA_ID = "forge-game://schemas/traceability-graph/1.0.0"
PREDICATE_RESULT_SCHEMA_ID = (
    "forge-game://schemas/traceability-predicate-result/1.0.0"
)
...
ALLOWED_RELATION_PAIRS: dict[str, set[tuple[str, str]]] = {
    ...
    "blocked_by": {
        (kind, blocker)
        for kind in ("requirement", "nfr", "system", "feature", "task", "test")
        for blocker in ("debt", "system", "feature", "task", "asset")
    },
    "has_debt": {
        (kind, "debt")
        for kind in ("requirement", "nfr", "system", "feature", "task", "asset")
    },
    "owns": {
        (owner, target)
        for owner in ("system", "feature", "task")
...
class TraceabilityGraph:
    ...
    def _validate(self) -> None:
        self._schemas.validate(self.document, TRACEABILITY_GRAPH_SCHEMA_ID)
