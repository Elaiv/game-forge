...
- Advance a ready checkpoint with `workflow-prepare`. It emits either one immutable `PhaseInvocation` 1.3, one exact `GateRequest`, or a blocked checkpoint. `allowed_actions` are the phase permission ceiling; `required_actions` are the subset required for successful completion. A phase remains blocked unless every allowed action has an exact executable adapter; no boolean override may broaden that set. Inspect Doctor `workflow_readiness` for exact blockers. Do not invoke a standalone executor to bypass that checkpoint.
