...
- Execute records only through `adapter-plan-request/1.1.0` and `execution-request/1.1.0`. The filesystem adapter materializes documents from the sealed request, applies architecture/catalog/backlog/graph first and ProjectState last, journals every target, and rolls every prior target back on failure. Reconcile with `action-reconciliation-request/1.1.0` before any retry.
