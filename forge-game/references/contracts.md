...
- Treat `GateRequest.subject_refs` as the complete normative approval scope and `context_refs` as audit-only history. Never copy the accumulated RunState artifact history into the approval scope. A `review_package_ref`, when present, is itself a normative subject and its exact hash is approval-bound.
...
- Advance a ready checkpoint with `workflow-prepare`. It emits either one immutable layout-bound `PhaseInvocation` 1.5, one exact `GateRequest` 1.1, or a blocked checkpoint. Human gate guards are evaluated while the request is built and appear as typed `guard_results`; they are not decorative labels. `readiness: ready` requires every guard satisfied and no blocking reasons. `readiness: blocked` exposes only the workflow's `reject` decision. `allowed_actions` are the phase permission ceiling; `required_actions` are the subset required for successful completion. A phase remains blocked unless every required action has an exact executable adapter; an unavailable optional action appears in `degraded_phases` and fails closed only if selected. No boolean override may broaden either set. Inspect Doctor `workflow_readiness` for exact blockers and degradations. Do not invoke a standalone executor to bypass that checkpoint.
...
- A legacy GateRequest 1.0 remains readable for interruption recovery, but it cannot approve `bootstrap.architecture`; reject it and regenerate architecture review under the current HumanReviewPackage contract.
...
- For `bootstrap.architecture`, validate one `human-review-package/1.0.0` Artifact produced by `bootstrap.architecture_review`. It binds the current three project-map contracts and one deterministic Markdown view. Runtime compares exact system/module/dependency/flow/NFR/feature/slice/scenario coverage, cross-contract ownership and dependencies, current backlog refs, source refs, review verdict, recommended decision, required sections, unresolved risks, and payload bytes before the review may transition to the human gate.
